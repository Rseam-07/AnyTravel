...
- 40 项常规单元测试中 38 项通过、2 项联网用例按设计跳过；Android lint 和 Release 通用 APK 构建通过。
- 景点、住宿与去返程班次可以锁定；修改旅行条件前先展示影响，应用后保留锁定和已确认项目，并支持整次撤回。成人、儿童年龄、房间、老人及移动辅助偏好已进入保存、报价与费用模型。
