...
| 酒店与民宿位置 | iOS 并行搜索酒店、精品酒店、连锁酒店、民宿、公寓酒店与青旅，再合并 RollingGo 城市及行程景点锚点目录，去重后最多展示 40 家 | 地图卡片显示品牌、星级、设施、标签、景点均距与交通枢纽距离；支持综合、低价、近景点、近交通、实时价和官网筛选 |
| 酒店实时价格 | 两端支持 RollingGo MCP（配置密钥后）；iOS 伴随服务另支持用户自行登录后的携程与同程/艺龙持久化浏览器会话 | 渠道、每晚/总价口径、抓取时间、缓存过期、部分成功、登录/验证状态、品牌官网与购买入口 |
...
| 智能向导 | 伴随服务托管 GLM-5.3-Flash；iOS 也支持用户自定义 OpenAI 兼容 Base URL、模型与 Key | 回应以卡片浮在地图上；可设置目的地/出发地、日期/天数、人数、预算、节奏、交通、兴趣与住宿筛选，也可直接生成或聚焦已有地点；失败时回退本机解析 |
