...
下载：[iOS 0.5.2 无签名 IPA](https://github.com/Rseam-07/AnyTravel/releases/tag/v0.5.2) · [Android 0.4.0 APK](https://github.com/Rseam-07/AnyTravel/releases/tag/v0.4.0)
...
0.5.2 发布前验证结果：23/23 iOS 单元测试、7/7 XCUITest、9/9 Node 测试通过；iPhone 17 Pro 模拟器走通首次进入、四类方案、保存恢复、生成后删改景点、跨天复制、撤销重做、条件重算，以及去程/返程独立选择与费用联动，设备 Release 构建产物为 0.5.2（6）。2026-08-31 的联网实查中，“上海→苏州”和“苏州→上海”各返回 8 个可购车次，两边均带余票、展示价格和官方购买入口；RollingGo 对 3 个苏州酒店候选完成逐店实时查询，3 个均返回可展示的每晚价与购买链接。密钥只保存在被 Git 忽略的节点环境文件中。
