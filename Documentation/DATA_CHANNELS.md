...
`Backend/src/adapters/railway12306.mjs` 读取铁路 12306 的公开车站表、余票查询和席别价格。它优先主站对主站的可购车次，返回车次、发到时刻、耗时、余票、二等座或其他可用席别价格。节点只读查询，不会代替用户提交订单；购买始终回到官方页面。
...
交通结果包含：渠道、交通方式、班次号、始发与到达站、发到时间、耗时、价格、席别、余票、抓取时间和购买 URL。App 会把它与地图中的机场/车站及已选住宿匹配，再计算接驳距离。
