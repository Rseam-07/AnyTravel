...
这个伴随服务把 App 的候选酒店统一交给多个价格源，也负责托管自然语言行程控制器，并直接提供 Web 生产资源。浏览器 Cookie 与需要商家身份的凭据留在服务端。开发构建可从 Git 忽略的本机配置测试直连；1.0 的 iOS 与 Android Release 不包含供应商或模型 Key，只保存公共服务地址。部署与发布门禁见 [公共服务部署](../Documentation/PRODUCTION_SERVICE.md)。

## Cloudflare 公共部署

当前 Web 使用 Cloudflare Workers + Workers AI + SQLite Durable Objects。配置、密钥注入、手册同步与发布命令见 [公共服务部署](../Documentation/PRODUCTION_SERVICE.md)。默认模型由 Cloudflare AI 绑定提供；Node 部署仍可使用下述外部模型配置。
...
健康检查：`curl http://127.0.0.1:8787/health`。除了服务身份、托管智能向导与高德地点之外，还会逐项报告铁路、飞猪航班、`ctripSession`、`ctripFlights`、`tongchengSession`、`elongOpenAPI`、`oneBoundCtrip`、`accorOfficial` 与 `hiltonOfficial` 的接入状态，方便确认哪些通道真的接上了。开发时可在“高级设置”填写本机地址；正式版本使用应用预设，无需用户配置。
...
- 飞猪适配器使用公开 listingsearch 2.2，解析分币金额、成人含税价、跨日到达和中转段；平台要求验证时会返回 verification_required，不绕过限制。
...
- `DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL` 与 `DEEPSEEK_MODEL`：为 `POST /v1/assistant/interpret` 提供首选智能向导；默认模型是 `deepseek-chat`。未配置 DeepSeek 时可用 `ZAI_API_KEY`、`ZAI_BASE_URL` 与 `ZAI_MODEL`。两条云端路径都只允许返回白名单内的路线动作，服务端会再次校验地点名、预算和枚举值。没有 Key、余额不足、超时或模型返回错误结构时，服务会自动使用 `local-intent-v1` 提取明确的城市、日期、同行人、预算、节奏、兴趣和交通动作，并在响应中标注 `mode: local-fallback` 与降级原因。
- `AMAP_API_KEY`：供高德 Web 服务 v5 地点适配器使用。它必须是在高德控制台创建的“Web 服务”Key；节点请求 `show_fields=business` 以取得可用的营业时间、评分与消费字段。iOS SDK Key 会返回平台不匹配，此时节点自动改查 OpenStreetMap/Nominatim，并将 `sourceCRS` 与 `outputCRS` 明确标成 `WGS84`；高德网络失败或零结果也走同一回退链路。
...
地点补充接口是 `POST /v1/places/search`。高德原始 GCJ-02 坐标与反算后的 WGS84 坐标语义都会随响应保留；开放地图回退则直接返回 WGS84。客户端只合并坐标来源与输出坐标系组合明确的结果。
...
- 2026-09-06 再次联网验证当前 GLM 凭据时，上游返回 HTTP 402 余额不足。相同完整中文需求随后由 `local-intent-v1` 返回 12 个白名单动作，正确提取宁波到苏州、9 月 12 至 14 日、3 人、每人 3000 元、轻松节奏、园林、美食和生成行程；响应明确标注 `upstream_http_402`，没有冒充云端成功。
- 当前收到的高德 Key 在官方 Web 服务请求中返回 `USERKEY_PLAT_NOMATCH (10009)`。2026-09-06 的相同请求已自动回退到 OpenStreetMap/Nominatim，并为“拙政园”返回 2 个 WGS84 候选。换成“Web 服务”Key 后会优先恢复高德业务字段，无需修改客户端。
- RollingGo 未配置密钥时会返回 `disabled` 诊断，不会生成占位价格。
- 去哪儿景点门票适配器已经落地；去哪儿酒店与航班仍只提供明确的渠道查询入口。同程酒店适配器已落地，但是否返回数字价格取决于本地登录会话和平台验证状态。
- 2026-09-06 后端单元测试为 64/64；同源 Web 托管、服务身份、自然语言降级、高德/OSM 坐标来源和飞猪中转航班都有回归覆盖。苏州 2026-09-12 至 13 的联网住宿请求返回 29 家、其中 26 家有数字价格，来自 RollingGo 与酒店官网；宁波往返苏州 2026-09-12 至 14 的联网交通请求返回 18 个有数字价格的去返程选项，来源为 12306 与飞猪。艺龙开放平台、万邦携程目录与携程航班适配器已有解析级覆盖，但本机未配置 `ELONG_*`、`ONEBOUND_*` 凭据，也未运行携程航班采集，因此这些通道还没有联网数字报价验收；健康检查会如实显示 `disabled`。
