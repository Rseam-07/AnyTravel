...
这个伴随服务把 App 的候选酒店统一交给多个价格源，也负责托管自然语言行程控制器。浏览器 Cookie 与需要商家身份的凭据留在自建节点。iOS 与 Android 0.8.1 的发布构建另可从 Git 忽略的本机配置注入 RollingGo、高德与智谱默认 Key，让用户不运行节点也能使用基础价格与智能向导；公开仓库不保存这些明文值。
