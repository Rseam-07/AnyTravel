...
- 2026-09-05 后端单元测试为 58/58；同源 Web 托管、服务身份健康检查和飞猪去返程航班适配器已有回归覆盖。RollingGo、雅高集团官网和希尔顿官网已经并入住宿聚合。雅高联网用例以所选日期取得大于零的每晚价、总价与官网购买入口；希尔顿目录价会明确标成参考价。艺龙开放平台、万邦携程目录与携程航班适配器已有解析级覆盖，但本机未配置 `ELONG_*`、`ONEBOUND_*` 凭据，也未运行携程航班采集，因此这些通道还没有联网数字报价验收；健康检查会如实显示 `disabled`。
