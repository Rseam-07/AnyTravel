(() => {
  ...
  async function mutateData(mutator, options = {}) {
    // The parent persists asynchronously. Prevent typing the next entry into
    // controls that the completed save is about to replace.
    const controls = [...ledgerRoot.querySelectorAll("button, input, select, textarea")].filter(control => !control.disabled);
  ...
  const publicApi = {
    // AnyTravel bridge: expose the unchanged integer-cent algorithm for exports and regression checks.
    calculateStatistics(snapshot) {
      const previous = ledgerData;
      try { ledgerData = normalizeData(snapshot); return deepClone(calculateStats()); }
      finally { ledgerData = previous; }
    },
    init,
    setActiveTab,
